...
- [x] `Apply` 整表重建语义；`Open` 不误删业务 Sheet
- [x] `Each` 默认并发 1，且始终 `SetLimit`
...
以后要「双表头 / 数据从第 N 行」：实现 `Layout`（含对称的 `ResolveHeaders` / `RenderHeaders`），不改 Schema、不改绑定核心。
...
`Apply()` 是**整表重建**：会先清掉目标 Sheet 上的旧数据行、数据验证，以及本库写入时生成的选项辅助表，再写出本次 notice / 表头 / 数据。同一 Sheet 上第二次 `Apply`（包括 `Template()`）不会残留上次的数据行。
...
`New()` 创建的空白 `Sheet1` 在写出其他表名时可能被清理；`Open()` 打开的已有文件**不会**自动删除任何 Sheet。
...
- `Each` 默认并发度为 **1**（串行）。`Concurrency(n)` / `SetConcurrency(n)` 控制 worker 上限；显式 `Concurrency(1)` 可覆盖 builder 上的 `SetConcurrency`。
- `File()` 是逃逸口：返回的 `*excelize.File` **不受**库锁保护，不要和并发中的库 API 混用同一句柄。
- `Open(r)` 把 `r` 交给 excelize 读取，**不**关闭 `r`；大文件可用 `WithUnzipSizeLimit` / `WithUnzipXMLSizeLimit`。`*os.File` 请自行 `Close`。记得 `wb.Close()`。

### 依赖策略

v2 锁定 **Go 1.22+**。底层 Excelize 保持 `v2.9.0`：上游 `v2.9.1+` / `v2.10+` 会抬高 Go 版本要求，升级前需同步评估本模块的 `go` 行。
