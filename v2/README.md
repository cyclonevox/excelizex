...
实际业务里最烦的往往不是读写单元格，而是：Excel 给你的是一堆 string，你手里却已经有一套带 tag、带校验、还可能嵌套的 DTO。以前为了一张导入表，controller / service 外面经常要写很多重复代码。并且无法使用已有的验证。这个库就是想把那段东西收成链式调用。
...
v2 **没有**兼容层，import 路径也不一样。建议新表直接用 v2；老项目一张表一张表迁，别指望一把梭。
